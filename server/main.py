...
def normalize_oab(value: Optional[str]) -> Optional[str]:
    if not value:
        return None
...
def is_subscription_active(
    ...
    if BILLING_MODE == "registration_only":
        return is_registered(user_id, stripe_email, oab_number)
...
def is_registered(user_id: str, email: Optional[str], oab_number: Optional[str]) -> bool:
    if REGISTRATION_BACKEND == "stripe" and STRIPE_SECRET_KEY and email:
        try:
            ...
            if not oab_number:
                return False
            normalized_oab = normalize_oab(oab_number)
            for customer in result.data:
                meta_oab = customer.metadata.get("oab_number") if customer.metadata else None
                if normalize_oab(meta_oab) == normalized_oab:
                    return True
            return False
        except Exception:
            return False
    with sqlite3.connect(DB_PATH) as conn:
        if user_id:
            cur = conn.execute(
                "SELECT oab_number FROM registrations WHERE user_id = ? AND status = 'active'",
                (user_id,),
            )
            row = cur.fetchone()
            if row and oab_number:
                return normalize_oab(row[0]) == normalize_oab(oab_number)
        if email:
            cur = conn.execute(
                "SELECT oab_number FROM registrations WHERE email = ? AND status = 'active'",
                (email,),
            )
            row = cur.fetchone()
            if row and oab_number:
...
def register_form(user_id: str = "", email: str = "") -> str:
    ...
    <html>
      ...
      <body>
        ...
        <p><strong>Primeiro cadastro:</strong> nome, email, OAB e UF.</p>
        <p><strong>Depois do cadastro:</strong> o login e validacao no GPT usam apenas <strong>email + OAB</strong>.</p>
        ...
        try:
            ...
            if existing.data:
                ...
                if cpf:
                    metadata["cpf"] = cpf
                if oab_number:
                    metadata["oab_number"] = oab_number
                if oab_state:
                    metadata["oab_state"] = oab_state
